...
sudo crudini --set /etc/keystone/keystone.conf database connection "mysql+pymysql://keystone:0penstack@open/keystone"
...
sudo su -s /bin/sh -c "keystone-manage bootstrap --bootstrap-password 0penstack \
  --bootstrap-admin-url http://open:5000/v3/ \
  --bootstrap-internal-url http://open:5000/v3/ \
  --bootstrap-public-url http://open:5000/v3/ \
...
export OS_AUTH_URL=http://open:5000/v3
...
sudo crudini --set /etc/glance/glance-api.conf database connection "mysql+pymysql://glance:0penstack_@open/glance"
sudo crudini --set /etc/glance/glance-api.conf keystone_authtoken auth_uri "http://open:5000"
sudo crudini --set /etc/glance/glance-api.conf keystone_authtoken auth_url "http://open:5000"
sudo crudini --set /etc/glance/glance-api.conf keystone_authtoken memcached_servers "open:11211"
